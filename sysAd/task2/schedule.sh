...
if [[ $(grep "$s" /server/future.txt) ]] ; then 
  ...
  grep “$s” /server/future.txt > /home/Jay_Jay/schedule.txt
  for i in {01..30};do
    ...
    grep "$s" /server/future.txt >> /home/sysAd_$i/schedule.txt 
    grep "$s" /server/future.txt >> /home/webDev_$i/schedule.txt 
    grep "$s" /server/future.txt >> /home/appDev_$i/schedule.txt
  done
